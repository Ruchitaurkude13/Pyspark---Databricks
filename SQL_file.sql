...
('Ruchita Urkude', '123456789012', '8876543210', '2000-05-15', '123 Street, Pune', 'H', 'MH'),
('Rani', '123654321098', '9812345678', '1995-08-22', '456 Road, Bangalore', 'O', 'KA'),
('Trupti', '123890123456', '8234567890', '1998-12-10', '789 Avenue, Mumbai', 'B', 'MH');
...
SELECT 
    ...
    AVG(AverageRate) AS average_rate
FROM Sales.CurrencyRate
WHERE FromCurrencyCode = 'USD'
AND ToCurrencyCode IN ('DZD', 'AUD')
group by ToCurrencyCode ,FromCurrencyCode

--2. Find the products having offer on it and display product name , safety Stock Level, 
--Listprice,  and product model id, type of discount,  percentage of discount,  
...
select p.name, p.safetyStockLevel, p.Listprice, p.productmodelid,
o.type, o.discountPct, o.startdate, o.enddate
from Sales.SpecialOffer o, Production.Product p, sales.SpecialOfferProduct op
where o.specialofferID = op.specialofferID and
...
select p.name, v.name, count(*) 
from Production.Product p,                        
Purchasing.ProductVendor pv,
Purchasing.Vendor v
where p.ProductID = pv.ProductID and
v.BusinessEntityID = pv.BusinessEntityID and
...
Purchasing.PurchaseOrderDetail pd,
Purchasing.PurchaseOrderHeader ph
where s.ShipMethodID = ph.ShipMethodID and
p.ProductID = pd.ProductID and
pd.PurchaseOrderID = ph.PurchaseOrderID and
s.name = 'ZY - EXPRESS';
...
select  s.Orderdate, p.ShipDate,p.taxamt
from Sales.SalesOrderHeader s, Purchasing.PurchaseOrderHeader p
where cast(p.shipdate as date) = cast(s.orderdate as date) and
s.shipmethodid = p.shipmethodid
...
select s.name, avg(DATEDIFF(DAY,h.orderdate,h.shipdate))
from Purchasing.PurchaseOrderHeader h, Purchasing.ShipMethod s
where s.ShipMethodID = h.ShipMethodID
group by s.Name
...
select name, avg(StandardCost)avgcost
...
from Person.Person pp, sales.SalesTerritoryHistory sh, sales.SalesTerritory st
where st.TerritoryID = sh.TerritoryID and
sh.businessEntityID = pp.businessEntityID

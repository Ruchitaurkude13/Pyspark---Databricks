---Q18. display empname,terriroty name,group,saleslastyear salesquota,bonus
...
---Q19. display empname,terriroty name,group,saleslastyear salesquota,bonus from Germeny and UK
Select(SELECT CONCAT_ws(' ',firstname,lastname) FROM Person.Person p 
...
select p.FirstName,p.LastName,datediff(year,BirthDate,HireDate) Age
from HumanResources.Employee e,
Person.Person p
where p.BusinessEntityID =e.BusinessEntityID
order by Age desc
...
select gender,avg(datediff(year,BirthDate,GETDATE())) average_age
